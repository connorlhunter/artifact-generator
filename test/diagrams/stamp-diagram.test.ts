...
describe("diagram stamps", () => {
  ...
  test("adds a high-contrast, source-owned version and date below the rendered diagram", () => {
    ...
    expect(stamped).toContain('viewBox="0 0 400 224"');
    ...
    expect(stamped).toContain('<rect x="0" y="200" width="400" height="24" fill="#111827"/>');
    expect(stamped).toContain('fill="#ffffff"');
    expect(stamped).toContain('<g id="diagram"/>');
    expect(stamped).toContain('x="390" y="212"');
  });
  ...
  test("keeps the stamp inside narrow diagram viewBoxes", () => {
    ...
    expect(stamped).toContain('viewBox="0 0 100 124"');
    expect(stamped).toContain('textLength="80" lengthAdjust="spacingAndGlyphs"');
    expect(smallest).toContain('viewBox="0 0 1 124"');
  ...
  test("scales stamp metrics from SVG width for wide, short diagrams", () => {
    ...
    expect(stamped).toContain('viewBox="0 0 2000 220"');
    expect(stamped).toContain('<rect x="0" y="100" width="2000" height="120" fill="#111827"/>');
    expect(stamped).toContain('x="1952" y="160"');
    expect(stamped).toContain('font-size="48"');
  });
